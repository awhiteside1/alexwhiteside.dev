  ...
  return (tree: Root) => {
    visitParents(tree, 'image', (node, parents) => {
      ...
      if(parent && grandParent && parent.type==="paragraph"){
        const parentIndex = grandParent.children.findIndex(item=>item===parent)
        const previousNode = grandParent.children[parentIndex-1]
        grandParent.children= replace(grandParent.children, node, (item)=>item===previousNode )
        grandParent.children= replace(grandParent.children, previousNode, (item)=>item===parent )
      }}
    }catch(e){
    }
    })
