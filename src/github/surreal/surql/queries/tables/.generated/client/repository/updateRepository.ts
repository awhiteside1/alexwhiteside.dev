...
export const updateRepository = async function (db: Surreal, id: RecordId ,repository: Partial<Repository>) {
  const _key = repositorySchema.pick({ id: true }).parse({ id });
  const payload = repositorySchema.omit({ id: true }).partial().parse(repository);
