...
export const addCompressedData = async (
	data: { embedding: number[]; original: string; repos: Array<string> }[],
) => {
	if (!tables) {
		tables = await init()
	}

	const table = makeArrowTable(data, {
...
export const summarizeData = async () => {
	...
	const repos = (await tables.repositories.toArrow())
		.toArray()
		.sort(byString((x) => x.name))
...
export const findRelatedRepos = async (query: string) => {
	...
	const topics = await tables.topics
		.search(query)
		.postfilter()
		.select(['text'])
		.limit(15)
